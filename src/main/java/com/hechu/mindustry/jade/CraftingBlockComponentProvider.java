...
public enum CraftingBlockComponentProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if (blockEntity instanceof MultiblockCraftingBlockEntity<?> craftingBlockEntity) {
            ...
            MindustryProcessingRecipe<?> recipeUsed = craftingBlockEntity.getRecipeUsed();
            if (recipeUsed != null) {
                recipeUsed.getIngredients().stream()
                        .map(ingredient -> (MindustryProcessingIngredient) ingredient)
                        .map(ingredient -> elements.item(ingredient.getItemStack(), 0.8f)
...
//            progressStyle.color2 = 0x000000 | 0x88 << 24;
                ...
                IElement icon = elements.item(recipeUsed.getResult(), 0.8f)
                        .size(new Vec2(16, 16)).translate(new Vec2(0, -2));
                tooltip.append(icon);
            }
        }
